...
myINFO="\
############################################
### MoonStack Installer for Debian (Sid) ###
############################################

Disclaimer:
This script will install MoonStack on this system.
...
# Install MoonStack dependencies
function fuGET_DEPS {
  export DEBIAN_FRONTEND=noninteractive
...
for i in "$@"
  do
	...
	echo "  Use this if you want to automatically deploy a MoonStack instance (--type=auto implied)."
        echo "  A configuration example is available in \"MoonStack/iso/installer/moon.conf.dist\"."
        echo
        echo "--type=<[user, auto, iso]>"
	echo "  user, use this if you want to manually install a MoonStack on a Debian (testing) machine."
...
if [ "$myMOON_DEPLOYMENT_TYPE" == "iso" ] || [ "$myMOON_DEPLOYMENT_TYPE" == "user" ];
  then
    myCONF_MOON_FLAVOR=$(dialog --keep-window --no-cancel --backtitle "$myBACKTITLE" --title "[ Choose Your MoonStack NG Edition ]" --menu \
...
cnpm install https://gitee.com/stackw0rm/elasticsearch-dump.git -g
pip3 install -i https://pypi.douban.com/simple --upgrade pip
hash -r
pip3 install -i https://pypi.douban.com/simple elasticsearch-curator yq

# Cloning MoonStack from GitHub and Set Docker Mirrors
fuBANNER "Cloning MoonStack"
git clone https://gitee.com/stackw0rm/MoonStack-v2.git /opt/moon
/opt/moon/iso/installer/set_mirror.sh http://f1361db2.m.daocloud.io
# Let's create the MoonStack user
fuBANNER "Create user"
addgroup --gid 2000 moon
adduser --system --no-create-home --uid 2000 --disabled-password --disabled-login --gid 2000 moon
...
case $myCONF_MOON_FLAVOR in
  STANDARD)
    fuBANNER "STANDARD"
    ln -s /opt/moon/etc/compose/standard.yml $myMOONCOMPOSE
  ;;
  SENSOR)
    fuBANNER "SENSOR"
    ln -s /opt/moon/etc/compose/sensor.yml $myMOONCOMPOSE
  ;;
  INDUSTRIAL)
    fuBANNER "INDUSTRIAL"
    ln -s /opt/moon/etc/compose/industrial.yml $myMOONCOMPOSE
  ;;
  COLLECTOR)
    fuBANNER "COLLECTOR"
    ln -s /opt/moon/etc/compose/collector.yml $myMOONCOMPOSE
  ;;
  NEXTGEN)
    fuBANNER "NEXTGEN"
    ln -s /opt/moon/etc/compose/nextgen.yml $myMOONCOMPOSE
  ;;
esac
...
echo "$myFAIL2BANCONF" | tee /etc/fail2ban/jail.d/moon.conf
...
tar xvfz /opt/moon/etc/objects/elkbase.tgz -C /
cp /opt/moon/host/etc/systemd/* /etc/systemd/system/
...
if [ "$myMOON_DEPLOYMENT_TYPE" == "iso" ];
  then
    usermod -a -G moon msec
  else
    usermod -a -G moon $(who am i | awk '{ print $1 }')
fi
chown moon:moon -R /data
chown msec:msec -R /home/msec/.ssh
...
# Final steps
cp /opt/moon/host/etc/rc.local /etc/rc.local && \
